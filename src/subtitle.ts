...
class SubRowList {
    ...
    insertEmpty(offset: number) {
        ...
        const currentRow = this.val[this.currentRowIndex]
        ...
        newSub.Style = currentRow.sub.Style
        if (offset == 0) {
            const preStart = this.val[this.currentRowIndex - 1 < 0 ? 0 : this.currentRowIndex - 1].sub.Start
            if (preStart >= currentRow.sub.Start - 3000 && preStart < currentRow.sub.Start)
                newSub.Start = preStart
            else
                newSub.Start = currentRow.sub.Start

            newSub.End = currentRow.sub.Start
        }
        else if (offset == 1) {
            newSub.Start = currentRow.sub.End

            const nextEnd = this.val[this.currentRowIndex + 1 >= this.val.length ? this.val.length - 1 : this.currentRowIndex + 1].sub.End
            if (nextEnd <= currentRow.sub.End + 3000 && nextEnd > currentRow.sub.End)
                newSub.End = nextEnd
            else
                newSub.End = currentRow.sub.End
        }
